...
async def analiseHistorico(request: Request):
    ...
    try:
        ...
        if result['status'] == 'sucesso':
            ...
            logger.error(f"Não processou: {result}")
            return JSONResponse(content={
                "status": "nao processado",
                "resultado": convert_sets(result['detalhes']),
                "payload": payload
        ...
        # Caso não esperado
        logger.error(f"Erro ao processar histórico {result}")
...
# ── Geração de Documentos Jurídicos (GerarPeticao) ────────────────────
...
@app.post("/documentos/gerar")
async def gerar_documento(request: Request):
    """
    Endpoint responsável por gerar e retornar documentos jurídicos (petições).

    Fluxo:
    1. Recebe tipo_documento, seções e dados do frontend
    2. Chama o módulo GerarPeticao para criar o .docx
    3. Retorna o arquivo .docx diretamente para download
    """
    ...
    try:
        ...
        if not dados or not isinstance(dados, dict):
            return err("Campo 'dados' é obrigatório", 400)

        # Flatten dados: extrai do dict aninhado {id: {var: valor}}
        dados_flat = {}
        ...
        # Extrai textos das subcategorias já prontos do HTML
        textos_secoes = []
        if secoes and isinstance(secoes, list):
            secoes_ordenadas = sorted(secoes, key=lambda s: s.get("ordem", 0))
            textos_secoes = [
                s["subcategoria"] for s in secoes_ordenadas
                if s.get("subcategoria")
            ]
        ...
        return err("Erro interno na geração do documento", 500, {"detail": str(e)})
